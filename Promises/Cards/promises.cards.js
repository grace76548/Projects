  ...
  // https://github.com/isabellaardon/JS-Promises-DeckofCards/blob/main/script.js
  // https://github.com/tmt16/card-deck-api-promises/blob/master/app.js
  
  async function drawOneCard() {
    try {
      console.log("Starting request to draw 1 card!"); 
      const response = await axios.get(`${baseURL}/new/draw/`);
          const { suit, value } = response.data.cards[0];
          console.log(`${value.toLowerCase()} of ${suit.toLowerCase()}`)
    } catch (error) {
      console.error("Error fetching 1 card data:", error);
    }
  }
  drawOneCard();
  ...
  async function drawTwoSequentiallly() {
    try {
      console.log("Starting request to draw 2 cards!");
      ...
      console.error("Error fetching 2 card data:", error);
    }
  }
